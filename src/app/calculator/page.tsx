  ...
  return (
    <div className="max-w-[1200px] mx-auto px-5 md:px-20 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-[#4A4A47] mb-6 print:hidden">
        ...
        <span className="text-[#1F4E5F] font-semibold">Deposit Return Calculator</span>
      </nav>
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 bg-[#EAF1EE] text-[#2E7D6B] text-xs font-bold rounded-full border border-[#2E7D6B]/20 flex items-center gap-1">
              <CalcIcon className="w-3.5 h-3.5" /> Client-Side Privacy Safe
            </span>
          </div>
          <h1 className="text-3xl font-bold text-[#1F4E5F] tracking-tight">Deposit Growth Calculator</h1>
          <p className="text-sm text-[#4A4A47] mt-1">
            Compute maturity schedules and compound interest projections with zero server tracking.
          </p>
        </div>
        ...
        <div className="bg-[#FAF9F5] border border-[#DADAD3] rounded-xl p-6 space-y-5 print:border-none print:p-0">
          <div>
            <label htmlFor={amountInputId} className="block text-xs font-semibold text-[#1F4E5F] mb-1.5">
              {calcType === "FD" ? "Lump Sum Initial Deposit (LKR)" : "Monthly Recurring Deposit (LKR)"}
            </label>
            <input
              id={amountInputId}
              type="number"
          ...
          <div>
            ...
            <select
              ...
              className="w-full p-2.5 bg-white border border-[#DADAD3] rounded-lg text-sm font-semibold text-[#1F4E5F] outline-none focus:ring-2 focus:ring-[#2E7D6B]"
            >
              <option value={0.25}>3 Months (0.25 Yrs)</option>
              <option value={0.5}>6 Months (0.5 Yrs)</option>
          ...
          {calcType === "FD" && (
            <div>
              ...
              <select
                ...
                className="w-full p-2.5 bg-white border border-[#DADAD3] rounded-lg text-sm font-semibold text-[#1F4E5F] outline-none focus:ring-2 focus:ring-[#2E7D6B]"
              >
                <option value={1}>Annually (At Maturity)</option>
        ...
        <div className="lg:col-span-2 bg-[#EAF1EE] border border-[#2E7D6B]/30 rounded-xl p-6 md:p-8 flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-[#4A4A47] mt-2">
              Based on {annualRate}% annual return over {tenureYears} {tenureYears === 1 ? "year" : "years"}.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-4 mt-8 pt-6 border-t border-[#2E7D6B]/20">
            <div>
              <span className="text-xs text-[#4A4A47] block">Total Principal Deposited</span>
            ...
            <div>
              <span className="text-xs text-[#4A4A47] block">Total Interest Yield</span>
      ...
      <div className="bg-[#FAF9F5] border border-[#DADAD3] rounded-xl overflow-hidden shadow-xs">
        <div className="p-4 bg-[#F4F3EE] border-b border-[#DADAD3] flex items-center justify-between">
          <h2 className="text-sm font-bold text-[#1F4E5F]">Milestone Growth Schedule</h2>
          <span className="text-xs text-[#4A4A47] flex items-center gap-1">
            <Info className="w-3.5 h-3.5 text-[#2E7D6B]" /> Indicative non-compounded withholding tax rates excluded
          </span>
        </div>
        <table className="w-full text-left text-sm border-collapse">
          <thead className="bg-[#EAF1EE] border-b border-[#DADAD3] text-[#1F4E5F]">
            <tr>
              <th className="p-3.5 font-bold">Period Milestone</th>
              <th className="p-3.5 font-bold">Principal Deposited</th>
              <th className="p-3.5 font-bold">Accrued Interest</th>
              <th className="p-3.5 font-bold">Total Estimated Value</th>
            </tr>
          </thead>
